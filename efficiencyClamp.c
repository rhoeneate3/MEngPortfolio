...
// struct to hold all the information returned from check_and_save_info()
typedef struct {
    double ecoll_freq;
    double icoll_freq;
    double density;
...
#define POP_SIZE 25  // Population size
#define GENERATIONS 25  // Number of generations
#define MUTATION_RATE 0.15  // 10% mutation chance
...
void solve_Poisson_ICP(xvector rho1, double tt) {
    ...
    for (i = 0; i < N_G; i++) {
        ...
        // Spatially varying induced E-field that mimics coil geometry
...
Info check_and_save_info(double timedifsec) {
    ...
    fprintf(f,"Number of Coil Turns                         = %12.3e \n", N_TURNS);
    ...
    else
    {
        ...
        // Compute input power
        double P_in = (V_rms * V_rms) / info.R_plasma;  //Input power at plasma interface where R_plasma is the resistance
...
double gui_power, gui_pressure, gui_gap, gui_temperature, gui_turns, gui_efficiency;
...
double mutate_pressure(double pressure) {
    ...
    if (((double)rand() / RAND_MAX) < MUTATION_RATE) {
        double mutation = ((double)rand()/RAND_MAX)* 6.0 - 3.0; // mutate by +/- 2Pa   
        pressure += mutation;     
...
double mutate_gap(double gap) {
    ...
    if (((double)rand() / RAND_MAX) < MUTATION_RATE) {
        double mutation = ((double)rand()/RAND_MAX)*1.0 - 0.5; // mutate by +/- 0.5mm     
        gap += mutation;      
    ...
    for (int gen = 0; gen < GENERATIONS; gen++) {
        for (int i = 0; i < POP_SIZE; i++) {
            ...
            *(config->param_ptr) = population[i];
            ...
            if (efficiency > 100.0 || efficiency < 0.0 || isnan(efficiency) || isinf(efficiency)) {
                fitness[i] = -1.0;
            } else {
...
// Modified Optimisation runner with simulation type selection
__declspec(dllexport)
void run_optimisation(const char* folder_path, char mode, char sim_type, int arg1, double target_efficiency, double power, double pressure, double gap, double temp, double turns, double min_power, double max_power, double min_pressure, double max_pressure, double min_l, double max_l) {
    ...
    gui_efficiency = TARGET_EFFICIENCY = target_efficiency;
    ...
    printf(">> [opt.c] Running simulation WITHOUT Optimisation.\n");
    measurement_mode = true;

    //calculate driving voltage from input power
    ...
    if (arg1 == 0) {
        // Initialisation path (same as optimisation mode)
        ...
        // Full run without optimisation loop
    ...
    printf(">> opt.c bounds are: min power = %.3f, max power = %.3f, min pressure = %.3f, max pressure = %.3f, min temp = %.1f, max temp = %.1f, min gap = %.4f, max gap = %.4f\n",
        MIN_POWER, MAX_POWER, MIN_PRESSURE, MAX_PRESSURE, MIN_L, MAX_L);

...
class CCPApp(tb.Window):  # Bootstrap themed window
    ...
    def create_widgets(self):
        ...
        # Add Number of Coils (no min/max)
        row_offset = len(params) + 1
        tb.Label(param_frame, text="Number of Coils:").grid(row=row_offset, column=0, sticky='w', pady=6, padx=5)
        tb.Entry(param_frame, textvariable=self.param_vars['turns'], width=10).grid(row=row_offset, column=1, padx=5)
        ...
        self.opt_type_dropdown = tb.Combobox(
            ...
            values=["Power", "Pressure", "Gap Length", "Temperature"],
            state="readonly",
        )
        ...
        # Cycles
        tb.Label(control_tab, text="Cycles:").grid(row=2, column=0, sticky="w", padx=5, pady=10)
        self.cycles_var = tk.StringVar(value="1000")
        tb.Entry(control_tab, textvariable=self.cycles_var, width=12).grid(row=2, column=1, sticky="w", padx=5, pady=10)
    ...
    def get_all_parameter_bounds(self):
        ...
        bounds = {}
        try:
            for key in ['power', 'pressure', 'gap_length', 'temperature']:
                bounds[key] = {
    ...
    def _run_simulation_phase(self, init=False, opt_param="None"):
        ...
        try:
            ...
            result = self.ccp.run_simulation(
                ...
                cycles,
                params['power'], params['pressure'], params['gap_length'], params['temperature'], params['turns'],
                bounds["power"]["min"], bounds["power"]["max"],
                bounds["pressure"]["min"], bounds["pressure"]["max"],
                bounds["gap_length"]["min"], bounds["gap_length"]["max"],
                bounds["temperature"]["min"], bounds["temperature"]["max"],
            )
